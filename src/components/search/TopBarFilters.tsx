...
import { Filter, MapPin, Gavel, Building, Clock } from 'lucide-react';
...
export const TopBarFilters = ({
  ...
  return <div className="flex items-start gap-3 min-w-0">
      ...
      {/* Filtro Formato */}
      <div className="flex-shrink-0">
        <FilterChip label={format ? `${formatOptions.find(opt => opt.value === format)?.label}` : "Formato"} selectedValue={format ? formatOptions.find(opt => opt.value === format)?.label : undefined} isActive={!!format} onClear={() => setFormat('')} className="w-[280px]" aria-label="Filtro de formato do leilão" id="format-filter">
          {({
          close
              ...
              <div className="flex flex-col gap-2">
                {formatOptions.map(option => <button key={option.value} onClick={() => handleFormatSelect(option.value, close)} className={cn("w-full px-4 py-3 text-left rounded-lg border transition-all duration-200 font-medium", format === option.value ? "bg-blue-50 border-blue-200 text-blue-700 ring-1 ring-blue-200/50" : "bg-white border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300")} style={{
      ...
      <div className="flex-shrink-0">
        <FilterChip label="Origem" selectedItems={getSelectedOriginLabels()} isActive={origins.length > 0} hasMultiple={true} onClear={() => setOrigins([])} onRemoveItem={removeOrigin} onSelectAll={handleSelectAllOrigins} className="w-[280px]" aria-label="Filtro de origem do leilão" id="origin-filter">
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              {originOptions.map(option => <button key={option.value} onClick={() => handleOriginToggle(option.value)} className={cn("w-full px-4 py-3 text-left rounded-lg border transition-all duration-200 font-medium", origins.includes(option.value) ? "bg-blue-50 border-blue-200 text-blue-700 ring-1 ring-blue-200/50" : "bg-white border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300")} style={{
              borderRadius: designTokens.borderRadius.lg,
              padding: `${designTokens.spacing.md} ${designTokens.spacing.lg}`
            }}>
                  {option.label}
      ...
      <div className="flex-shrink-0">
        <FilterChip label="Etapa" selectedItems={getSelectedStageLabels()} isActive={stages.length > 0} isDisabled={format === 'venda-direta'} hasMultiple={true} onClear={() => setStages([])} onRemoveItem={removeStage} onSelectAll={handleSelectAllStages} className="w-[280px]" aria-label="Filtro de etapa do leilão" id="stage-filter">
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              {stageOptions.map(option => <button key={option.value} onClick={() => handleStageToggle(option.value)} className={cn("w-full px-4 py-3 text-left rounded-lg border transition-all duration-200 font-medium", stages.includes(option.value) ? "bg-blue-50 border-blue-200 text-blue-700 ring-1 ring-blue-200/50" : "bg-white border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300")} style={{
              borderRadius: designTokens.borderRadius.lg,
              padding: `${designTokens.spacing.md} ${designTokens.spacing.lg}`
            }}>
                  {option.label}
      ...
      {getFilterCount() > 0 && <div className="flex-shrink-0 flex items-center">
          <div className="px-3 py-2 bg-blue-100 text-blue-700 rounded-full text-sm font-medium" style={{
